      ...
      >
        ...
        >
          {expandedItems?.includes(value)
            ? (openIcon ?? <FolderOpenIcon weight="bold" className="size-4 text-blackfont-500" />)
            : (closeIcon ?? <FolderIcon weight="bold" className="size-4 text-blackfont-500" />)}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
    ...
    return (
      <AccordionPrimitive.Item value={value} className="relative">
        <AccordionPrimitive.Trigger
          ...
          aria-label="File"
          className={cn(
            'flex w-full items-center gap-1 cursor-pointer text-sm text-blackfont-500 pr-1 rtl:pl-1 rtl:pr-0 rounded-md  duration-200 ease-in-out',
            {
              'bg-grey-500': isSelected && isSelectable
            },
